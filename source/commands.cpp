...
u64 getMainNsoBase(u64 pid, Result &ldrDmntResult){
    LoaderModuleInfo proc_modules[2];
    s32 numModules = 0;

    ldrDmntResult = ldrDmntInitialize();
...
MetaData getMetaData(Result &ldrDmntResult){
    MetaData meta;
    attach();
    u64 pid = 0;    
    Result rc = pmdmntGetApplicationProcessId(&pid);
    ...
    meta.main_nso_base = getMainNsoBase(pid, ldrDmntResult);
...
std::string peek(u64 offset, u64 size) {
    u8 *out = new u8[size];
    ...
    for (u64 i = 0; i < size; i++)
        ss << std::setw(sizeof(u8)*2) << int(out[i]);
...
{
    printf("[debug#clickSequence] char* seq: %s\t|| *token: %d\n", seq, *token);
    // clickSeq A,W1000,DR,W1000,A,W1000,DR,W1000,A,DL,W1000,DL,W1000,DD,W1000,A,W1000,DU,W1000,A,W1000,DR,W1000,A,W1000,DR,W1000,A,W1000,DL,W1000,DL,W1000,DD,W1000,A,W1000
    ...
    while (command != NULL)
    {
        ...
        {
            // click
            printf("[debug] Current clickSeq command: %s\t|| strcmp(command, \"DR\"): %d || strcmp(command, \"DRIGHT\"): %d\n", command, strcmp(command, "DR"), strcmp(command, "DRIGHT"));
